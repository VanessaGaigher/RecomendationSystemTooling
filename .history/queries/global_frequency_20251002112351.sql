...
JOIN OSUSR_230_TOOLINGTYPE tt ON tt.ID = ti.TYPEID
JOIN OSUSR_230_TOOLINGFAMILY tf ON tf.ID = ti.FAMILYID
JOIN OSUSR_230_TOOLINGSUBFAMILY sf ON sf.ID = ti.SUBFAMILYID
WHERE ti.INTERNALCOMPANYID = 1
    ...
    AND tm.STATUSID = 2
GROUP BY tt.CODE + '.' + tf.CODE + '.' + sf.CODE
